...
Historical record only. Native-card claims varied between candidate and stable runs; this report does not establish v0.3.1 acceptance. See [current validation](platform-validation.md) for version-bound results.
...
|---|---|---|---|---|---|---|---|
| Codex | ✅ | ✅ Previous RC4 copy moved recoverably to Trash before the stable package was installed | ✅ Stable v0.3.0 Codex ZIP installed to the personal Skill directory and entrypoint checked | ✅ Fresh Codex CLI 0.154.0 loaded the stable installed Skill and ran its real `workflow-entry` | ✅ Stable first turn was table-first `PRECHECK-WIFI-Q1`; the installed harness rejected `继续` with attempts = 1 and retained the same options | ✅ The stable installed harness accepted `0`; final report recorded no TV operation | ⏭️ App selection, Home, wallpaper, ADB shutdown and physical TV not run |
| WorkBuddy 中国大陆版 | ✅ | ✅ Previous RC4 target moved recoverably to Trash before stable installation | ✅ Stable v0.3.0 WorkBuddy ZIP installed through a hash-verified local filesystem fallback and entrypoint checked | ✅ A new WorkBuddy 5.5.6 task loaded the stable installed Skill | ✅ Stable live acceptance: table-first `PRECHECK-WIFI-Q1`; exact `continue` rejected with a ❌ row and the same four choices retained | ✅ Stable live reply `0` reached the terminal state; final table recorded no TV operation | ⏭️ App selection, Home, wallpaper, ADB shutdown and physical TV not run |
...
- A fresh official Codex CLI 0.154.0 process loaded the stable installed Skill, initialized `codex / local_computer / text_menu`, and ran the real `workflow-entry`. It displayed the table-first `PRECHECK-WIFI-Q1`; the same installed harness then rejected `继续` with exit code 2 and a ❌ row, accepted `0`, and generated a final report stating that no television operation ran.
- WorkBuddy stable replacement/entrypoint checks are current. A new stable-package task appeared as `Android TV Apps Helper 只读预检查`, loaded the installed Skill, and displayed the table-first `PRECHECK-WIFI-Q1`. Exact `continue` produced the ❌ invalid-answer row while retaining the same question and four options. Reply `0` generated `final-report.md` and a final table stating that no television operation ran. During the first render the client temporarily showed a blank result pane, but the running task was preserved and its completed result became visible after reopening it.
- The fixes added after RC4—session-start launcher restore binding, exact prechecked ADB path binding, and non-duplicated finish choices—are covered by the final automated suite rather than a physical-TV run.
